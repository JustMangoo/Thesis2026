  ...
  <div class="p-6">
    ...
    <VueDraggable v-model="sections" handle=".drag-handle" :disabled="readonly" class="space-y-3">
      <div v-for="(section, idx) in sections" :key="section.id"
        ...
        <div class="flex items-center gap-2 px-3 py-2 bg-neutral-50 border-b border-neutral-100">
          <span v-if="!readonly" class="drag-handle cursor-grab text-neutral-300 hover:text-neutral-500">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path
                d="M8 6a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm8 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM8 12a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm8 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM8 18a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm8 0a1.5 1.5 0 110 3 1.5 1.5 0 010-3z" />
            </svg>
          </span>
          <span v-if="version?.numbered_sections" class="text-sm font-medium text-neutral-500 w-5">{{ idx + 1 }}.</span>
          <input :value="section.title" :disabled="readonly" placeholder="Heading"
          ...
          <button v-if="!readonly" class="text-neutral-300 hover:text-danger transition-colors"
        ...
        <PolicyEditor :model-value="section.content" :editable="!readonly"
        ...
        <div v-if="section.footnote || showFootnote[section.id]" class="px-3 py-2 border-t border-neutral-100">
          <textarea :value="section.footnote" :disabled="readonly" placeholder="Add footnote..." rows="2"
        ...
        <div v-if="!readonly && !section.footnote && !showFootnote[section.id]"
          class="px-3 py-2 border-t border-neutral-100">
          <button class="text-xs text-primary hover:underline" @click="toggleFootnote(section.id)">+ Add
            footnote</button>
        </div>
      </div>
    </VueDraggable>
    ...
    <button v-if="!readonly"
      class="mt-3 w-full py-2.5 border border-dashed border-neutral-300 rounded-sm text-sm text-neutral-500 hover:border-primary hover:text-primary transition-colors flex items-center justify-center gap-1"
      @click="addSection">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
